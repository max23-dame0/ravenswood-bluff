...
def build_fresh_orchestrator(force_backend_mode: str | None = None) -> GameOrchestrator:
    ...
    state = GameState(
        ...
        players=(PlayerState(player_id="h1", name="Host", role_id="washerwoman", team=Team.GOOD),),
    )
...
async def ensure_game_loop_running() -> bool:
    ...
    async with _game_lock:
        ...
        global_game_loop_game_id = (
            global_orchestrator.state.game_id if global_orchestrator else None
        )
...
def collect_metrics(orchestrator: GameOrchestrator) -> dict[str, Any]:
    ...
    nomination_prompted = [
        e for e in events if e.event_type in {"nomination_prompted", "nomination_window_opened"}
    ]
    ...
    if last_phase_event:
        phase_duration_seconds = round(
            (datetime.now() - last_phase_event.timestamp).total_seconds(), 2
        )
    ...
    return {
        ...
        "loop_task": {
            ...
            "cancelled": bool(global_game_loop_task.cancelled())
            if global_game_loop_task
            else False,
            "game_id": global_game_loop_game_id,
            "started_at": global_game_loop_started_at.isoformat()
            if global_game_loop_started_at
            else None,
        ...
        "judgements": orchestrator.storyteller_agent.get_recent_judgements(limit=40)
        if orchestrator.storyteller_agent
        else [],
        "latest_execution": last_execution,
        "storyteller_delegated": getattr(orchestrator.storyteller_agent, "delegated", False)
        if orchestrator.storyteller_agent
        else False,
        "storyteller_mode": getattr(orchestrator.storyteller_agent, "mode", "manual")
        if orchestrator.storyteller_agent
        else "manual",
        "runtime": runtime,
...
def filter_chat_history(
    orchestrator: GameOrchestrator, player_id: str | None, viewer_mode: str
) -> list[dict[str, Any]]:
    if not orchestrator:
        return []
    ...
    for m in history:
        ...
        if not m.recipient_ids:
            # 公开消息
            visible = True
        elif player_id in (m.speaker, m.target_player) or (
            m.recipient_ids and player_id in m.recipient_ids
        ):
            # 涉及自己的私聊
            visible = True
            ...
            evil_ids = [
                p.player_id
                for p in orchestrator.state.players
...
def build_private_info_list(
    orchestrator: GameOrchestrator, player_id: str | None
) -> list[dict[str, Any]]:
    if not player_id:
        return []
    messages = [
...
def build_nomination_state(orchestrator: GameOrchestrator) -> dict[str, Any]:
    ...
    terminal_stage = nomination_state.get("stage", "idle") in {
        "executed",
        "no_nomination",
        "invalid_nomination",
    }
    terminal_result = nomination_state.get("result_phase") in {
        "execution_resolved",
        "no_nomination",
        "invalid_nomination",
    }
                ...
                inferred_history.append(
                    {
                        ...
                        "round": len(
                            [
                                ...
                                if item.get("kind") == "nomination_started"
                            ]
                        )
                        + 1,
                ...
                inferred_history.append(
                    {
                        ...
                        "round": len(
                            [
                                item
                                for item in inferred_history
                                if item.get("kind") == "voting_resolved"
                            ]
                        )
                        + 1,
                        "nominee": event.target,
                ...
                inferred_history.append(
                    {
                        ...
                        "round": len(
                            [
                                item
                                for item in inferred_history
                                if item.get("kind") == "execution_resolved"
                            ]
                        )
                        + 1,
    ...
    nomination_state["has_current_round"] = bool(
        not is_terminal
        and (
            nomination_state.get("stage")
            in {"window_open", "nomination", "defense", "voting", "resolved"}
...
async def websocket_endpoint(websocket: WebSocket, player_id: str):
    ...
    async def send_message_with_session(message: str) -> None:
        await manager.send_personal_message(
            decorate_ws_message_with_game_id(message, global_orchestrator), player_id
        )
...
async def setup_game(data: dict[str, Any]):
    ...
    storyteller_client_id = data.get("storyteller_client_id") or (
        human_client_id if human_mode == "storyteller" else None
    )
    ...
    for pid in seat_order:
        ...
        if p:
            ...
            p_info = {
                ...
                "ghost_votes_remaining": getattr(p, "ghost_votes_remaining", 1)
                if not p.is_alive
                else 0,
            }
...
async def storyteller_night_next():
    ...
    return await global_orchestrator.storyteller_agent.get_human_storyteller_step(
        global_orchestrator.state, phase
    )
