...
SCRIPTS = {"trouble_brewing": TROUBLE_BREWING}
...
def get_role_counts(player_count: int) -> dict:
    ...
    return {"townsfolk": counts[0], "outsider": counts[1], "minion": counts[2], "demon": counts[3]}
...
def distribute_roles(script: ScriptConfig, player_count: int) -> tuple[list[str], list[str]]:
    ...
    pools = {RoleType.TOWNSFOLK: [], RoleType.OUTSIDER: [], RoleType.MINION: [], RoleType.DEMON: []}
    ...
    selected.extend(
        random.sample(
            pools[RoleType.OUTSIDER], min(counts["outsider"], len(pools[RoleType.OUTSIDER]))
        )
    )
    selected.extend(
        random.sample(
            pools[RoleType.TOWNSFOLK], min(counts["townsfolk"], len(pools[RoleType.TOWNSFOLK]))
        )
    )
    ...
    bluff_candidates = [
        ...
        if get_role_class(r).get_definition().role_type in (RoleType.TOWNSFOLK, RoleType.OUTSIDER)
        and r not in in_play
    ]
