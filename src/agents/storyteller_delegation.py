"""
StorytellerAgent 委托 / 逻辑层（从 storyteller_agent.py 抽出）

设计目标（见 `DECISIONS.md` D006）：facade 仅做路由，说书人裁量逻辑下沉到本模块。
`StorytellerAgent` 继承 `StorytellerAgentDelegation`，所有方法仅通过 `self` 访问
实例属性，调用点保持 `self._x(...)` 不变，故行为完全一致。
"""
...
class StorytellerAgentDelegation:
    """说书人裁量逻辑（夜信息裁定 / 平衡样本 / 夜间编排 / 判决账本）。"""
