...
class EvilStrategy:
    ...
    def get_evil_strategic_summary(self, visible_state: AgentVisibleState) -> str:
        ...
        if monk_players:
            summary_parts.append(
                f"- **僧侣风险**: 玩家 {', '.join(monk_players)} 申明了僧侣。注意他们可能保护了核心信息位。"
            )
        if soldier_players:
            summary_parts.append(
                f"- **士兵风险**: 玩家 {', '.join(soldier_players)} 申明了士兵。恶魔刀他们将无效。"
            )
        if mayor_players:
            summary_parts.append(
                f"- **市长风险**: 玩家 {', '.join(mayor_players)} 申明了市长。攻击他们可能导致【杀戮转移】。"
            )

        # 2. 哑刀与失败回溯分析 (GAME-3.2)
        kill_events = [e for e in visible_state.visible_event_log if e.event_type == "night_kill"]
        death_events = [
            e for e in visible_state.visible_event_log if e.event_type == "player_death"
        ]
        ...
        if kill_events:
            ...
            for e in kill_events:
                ...
                actual_deaths = [
                    ...
                    if de.timestamp > e.timestamp
                    and de.phase == e.phase
                    and de.round_number == e.round_number
                ]

                target_died = any(de.target == intended_target for de in actual_deaths)
                if not target_died or e.payload.get("failed"):
                    failures_per_target[intended_target] = (
                        failures_per_target.get(intended_target, 0) + 1
                    )
                    ...
                    if actual_deaths and not target_died:
                        ...
                        redirections.append(
                            f"针对 {intended_target} 的攻击似乎被【转移】到了 {other_dead}"
                        )
            ...
            if is_last_failed:
                ...
                if last_failures >= 2:
                    summary_parts.append(
                        f"- **高危警报**: 你已经连续 {last_failures} 次尝试击杀 {target_name} 失败！这极大概率意味着该目标受到【隐形僧侣】的长期保护，或者是【士兵】。**请务必立即更换目标**。"
                    )
                elif redirections:
                    summary_parts.append(
                        f"- **转位警示**: {redirections[-1]}。这高度疑似【市长】在场导致的转位，请谨慎处理。"
                    )
                else:
                    summary_parts.append(
                        f"- **哑刀警示**: 昨晚对 {target_name} 的击杀尝试失败了。可能是僧侣保护、士兵或隐形成员。"
                    )
        ...
        if peaceful_nights >= 2 and not monk_players:
            summary_parts.append(
                f"- **隐性防御推断**: 本局已出现 {peaceful_nights} 次夜晚无死亡且无人申明僧侣。场上极大概率存在【隐形僧侣】或【士兵/市长】。建议优先刀掉那些表现活跃但一直没死的“稳坐”玩家。"
            )

        # 4. 战术建议
        if agent.role_id == "imp":
            my_trust = (
                agent.social_graph.get_profile(agent.player_id).trust_score
                if agent.social_graph.get_profile(agent.player_id)
                else 0
            )
            if my_trust < -0.6:
                summary_parts.append(
                    "- **战术建议**: 你目前疑点极高。考虑今晚“自杀”传位给爪牙（Star-pass）以洗清嫌疑。"
                )

        if agent.perceived_role_id in ["fortune_teller", "empath", "undertaker"]:
            summary_parts.append(
                f"- **伪装同步**: 你目前的伪装身份是 {get_role_name(agent.perceived_role_id)}。请确保护航此身份。"
            )
        ...
        if spy_book_summaries:
            if agent.role_id == "spy":
                summary_parts.append(
                    "- **间谍魔典**: 你拥有完整的角色信息。在邪恶频道中分享关键发现（如谁是僧侣/士兵），帮助恶魔选择击杀目标。"
                )
            else:
                summary_parts.append(
                    "- **队友情报**: 你的邪恶同伴（间谍）可能已经分享了场上角色信息，请关注邪恶频道中的情报。"
                )
    ...
    async def build_evil_night_coordination_message(
        ...
        target_name = (
            agent._player_name_from_visible_state(target_id, visible_state)
    ...
    def _build_template_coordination_message(
        ...
        if isinstance(target_id, str) and target_name:
            if role_id == "imp":
                ...
                msg_parts.append(
                    f"我今晚准备刀 {target_name}，这条线当前可疑度较高（{score:.2f}），白天可以顺着这条继续施压。"
                )
            elif role_id == "poisoner":
                priority = agent._poisoner_priority_for_target(target_id, visible_state)
                msg_parts.append(
                    f"我今晚准备毒 {target_name}，这人更像关键信息位（优先级 {priority:.2f}），白天看他的信息表现。"
                )
        else:
            if role_id in {"scarlet_woman", "spy", "baron"}:
                msg_parts.append(
                    "我今晚没有主动技能，但我会继续在白天带节奏，帮恶魔挡刀，或者找准好人漏洞进行攻击。"
                )
            else:
