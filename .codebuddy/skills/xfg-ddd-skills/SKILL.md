...
name: xfg-ddd-skills
description: >
  DDD 六边形架构（Domain/Case/Infrastructure/Trigger）设计模式、references 文档与 Docker 部署指引；
  含 Maven Archetype 脚本 `create-ddd-project.sh` 用于通用 DDD 工程。触发场景：DDD、领域驱动、聚合、
  值对象、端口适配器、仓储、创建 DDD 项目、Docker/容器化/上线部署等。
  TME 内部多模块工程请与 tme-scaffold-init、tme-starter-web、tme-starter-mybatis 等技能配合（见下节）。
version: 2.2.3
upstream: xiaofuge xfg-ddd-skills (MIT)
license: MIT
---

# DDD Hexagonal Architecture

## 与 TME 工程体系的关系（TME 项目必读）

- **新建 TME 内部系统**（`com.tme.it`、`tme-it-base`、`tme-base-dependencies`）：请优先使用技能包 **`tme-scaffold-init`** 初始化多模块骨架、Nacos、starter 选型；**不要**用本包脚本替代该流程。
- **本包定位**：DDD 分层、聚合/实体/值对象、仓储与端口适配、Case 编排；`references/*.md` 为设计与反模式指南。落地时可将 **Trigger 层** 对应 Spring MVC Controller，并结合 **`tme-starter-web`** + **`tme-starter-web-pro`**（须同时依赖两者）做统一返回、异常、校验与防重等。
- **脚手架差异**：`scripts/create-ddd-project.sh` 面向 **通用 Maven Archetype**，环境提示为 **JDK 17+**；TME 现网以公司脚手架与 JDK 版本为准，领域代码仍可按本包分层思想落在业务包下。
- **仓储与基础设施**：MyBatis 等实现可对照 **`tme-starter-mybatis`**；统一异常/返回见 **`tme-starter-common`** / **`tme-starter-web`**。
- **相关技能**：`tme-scaffold-init`、`tme-starter-web`、`tme-starter-web-pro`、`tme-starter-mybatis`、`tme-starter-common`、`tme-starter-nacos`。
...
**脚本执行方式**（在本技能包根目录下运行，即包含本 `SKILL.md` 的目录，例如全局技能目录 `~/.github/skills/xfg-ddd-skills/`）:
```bash
bash scripts/create-ddd-project.sh
```

> ⚠️ **必须先 cd 到上述技能包根目录再执行**，脚本会自动定位自身路径。
