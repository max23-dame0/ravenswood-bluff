...
| 指标 | 定义 | 目标值 | 采集方式 |
|------|------|:--:|------|
| **M1 玩家间指纹距离** | 同局两两玩家的行为指纹归一化欧氏距离均值 | **相对提升**（⚠️ 2026-08-07 T1 基线实测 mock 天然噪声即达 0.3212，绝对值被随机性污染 → 验收改为：同种子/同阵容下，注入差异化组 vs 无注入对照组的距离差 Δ>0 且显著；绝对值 0.30 仅作参考下限） | 行为指纹基准脚本（mock 固定种子，对照组对照） |
...
|:--:|:--:|:--:|:--:|------|------|
| **T1** | 行为指纹 | 🥇P0 | 🟢 已完成 | 行为指纹采集 + 基准脚本落地，量化**现状**玩家同质度 | T1.1 指纹向量 12 维、从 mock 对局 trace 可采集 ✅；T1.2 脚本输出两两距离矩阵 + 基准报告 ✅（5 局 8 人基线 mean_distance=0.3212，证据 `docs/alpha-1.2-evidence/pln040-t1-player-distinctness-baseline-2026-08-07.json`）；T1.3 `pytest` 新增指纹采集单测全绿 ✅（18 用例） |
| **T2** | 共享经验池 | 🥇P0 | 🟢 已完成 | 跨局经验池：沉淀 → 检索 → 按玩家视角注入 | T2.1 敏感过滤复用 ✅（`MemoryToolsLike.is_sensitive`，恶魔/队友名单拒绝入池，单测覆盖）；T2.2 同角色/阵营检索 ✅（角色 +3/阵营 +1/新鲜度，top_k 排序，12 单测全绿）；T2.3 前缀稳定 ✅（注入在 `load_player_profile` setup 时算一次，`token_budget_benchmark` RESULT: PASS system 1992 字符稳定） |
| **T3** | 差异化注入 | 🥈P1 | 🟢 已完成 | tendency 标定 + Archetype 联动，使差异**可感知** | T3.1 标定实验（M5）✅——baseline 0.3127 vs polarized 0.3430（Δ+0.030）vs mixed 0.3482（Δ+0.036），差异化生效，证据 `docs/alpha-1.2-evidence/pln040-t3-tendency-calibration-2026-08-07.md`；T3.2 指纹距离显著拉开（M1 相对对照 Δ>0）✅；T3.3 旧 profile 兼容 ✅（无 tendency 默认补齐，中性区间不覆盖保行为兼容） |
...
>
> **T1 基线结论（2026-08-07，5 局 8 人 mock，seed=42）**：
> - mean_distance=**0.3212**（已 ≥0.30），max=0.6226，min=0.0（存在完全同质玩家对）；
> - 但 mock fallback 率 ~57% 拉大随机噪声，**M1 绝对值阈值会被随机噪声污染** → T3 验收改判为"相对 T1 基线的同控制组对照"（同种子/同阵容下注入差异化后距离 vs 无注入），不以绝对值 0.30 为准。
> - 跨局稳定性 0.15~0.28（p2 最稳 0.1519、p3/p7 最不稳 ~0.28），为 M2 提供基线。此洞察需在 §3.2 指标表中补充说明。
