...
async def test_ghost_vote_conservation():
    ...
    print(
        f"Dead Player (1 vote) - Suspicion: {suspicion:.2f}, Threshold: {threshold:.2f}, Decision: {decision}"
    )
    assert not decision, "Dead player should conserve ghost vote for higher suspicion"
...
async def test_group_momentum():
    ...
    # 从众型玩家
    p = Persona(
        "从众者", "跟随大家", archetype="cooperative"
    )  # core registry assigns '从众' to cooperative
    agent = AIAgent("p1", "Alice", backend, p)
    ...
    state_empty = GameState(
        phase=GamePhase.VOTING, alive_count=8, current_nominee="p2", votes_today={}
    )
    _, _, thresh_empty = agent._select_vote_decision(state_empty)

    # 场景 B: 已有 5 人投票 (接近阈值)
    state_crowded = GameState(
        phase=GamePhase.VOTING,
        alive_count=8,
        current_nominee="p2",
        votes_today={"p3": True, "p4": True, "p5": True, "p6": True, "p7": True},
    )
    ...
    assert thresh_crowded < thresh_empty, (
        "Cooperative player should have lower threshold under group pressure"
    )
