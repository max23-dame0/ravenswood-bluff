...
def _check_balance_index(repo_root: Path) -> dict:
    ...
    if payload.get("curated_node_count", 0) < 5:
        raise SystemExit(
            "storyteller balance acceptance failed: curated node coverage is too small"
        )
    if payload.get("full_game_node_count", 0) < 10:
        raise SystemExit(
            "storyteller balance acceptance failed: full-game node coverage is too small"
        )
    ...
    if summary.get("suppressed_info_count", 0) < 1:
        raise SystemExit(
            "storyteller balance acceptance failed: missing suppressed storyteller samples"
        )
    if summary.get("distorted_info_count", 0) < 1:
        raise SystemExit(
            "storyteller balance acceptance failed: missing distorted storyteller samples"
        )
    ...
    if "judgement_category_counts" not in summary or not summary["judgement_category_counts"]:
        raise SystemExit(
            "storyteller balance acceptance failed: missing judgement category distribution"
        )
    ...
    if "judgement_bucket_counts" not in summary or not summary["judgement_bucket_counts"]:
        raise SystemExit(
            "storyteller balance acceptance failed: missing judgement bucket distribution"
        )
    if "adjudication_path_counts" not in summary or not summary["adjudication_path_counts"]:
        raise SystemExit(
            "storyteller balance acceptance failed: missing adjudication path distribution"
        )
    if "distortion_strategy_counts" not in summary or not summary["distortion_strategy_counts"]:
        raise SystemExit(
            "storyteller balance acceptance failed: missing distortion strategy distribution"
        )
    if not full_games:
    ...
    for game in full_games:
        ...
        if game_summary.get("event_node_fallback_count", 999) > 0:
            raise SystemExit(
                "storyteller balance acceptance failed: full-game nodes still rely on event-node fallback"
            )
        if not game_summary.get("judgement_bucket_counts"):
            raise SystemExit(
                "storyteller balance acceptance failed: per-game judgement bucket distribution is missing"
            )
        if (
            game.get("source") == "mock_full_game"
            and game_summary.get("night_info_judgement_count", 0) < 1
        ):
            raise SystemExit(
                "storyteller balance acceptance failed: mock full-game trace is missing real night_info judgement coverage"
            )
        ...
        if game_index.get("node_count") != game.get("node_count"):
            raise SystemExit(
                "storyteller balance acceptance failed: per-game node index is inconsistent"
            )
    ...
    if combined_suppressed < 1:
        raise SystemExit(
            "storyteller balance acceptance failed: full-game traces missing suppressed cases"
        )
    if combined_distorted < 1:
        raise SystemExit(
            "storyteller balance acceptance failed: full-game traces missing distorted cases"
        )
    if combined_legacy < 1:
        raise SystemExit(
            "storyteller balance acceptance failed: full-game traces missing legacy fallback cases"
        )
    return payload
...
def main() -> int:
    ...
    _run(
        repo_root,
        [
            str(repo_root / "scripts" / "export" / "storyteller_balance_sample_export.py"),
            "--full-games",
            "1",
            "--timeout-seconds",
            "30",
