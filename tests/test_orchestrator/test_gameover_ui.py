...
def test_history_overlay_contract_is_wired():
    ...
    assert (
        "fetch(`${backendHttpOrigin()}/api/game/history/player/${encodeURIComponent(currentUserId)}`)"
        in INDEX_HTML
    )
    assert (
        "fetch(`${backendHttpOrigin()}/api/game/history/${encodeURIComponent(gameId)}/player/${encodeURIComponent(currentUserId)}`)"
        in INDEX_HTML
    )
