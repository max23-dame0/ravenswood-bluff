...
from src.agents.memory.episodic_memory import Episode, EpisodicMemory
...
def test_episodic_memory():
    ...
    ep = Episode(
        phase=GamePhase.DAY_DISCUSSION, round_number=1, day_number=1, summary="第一天大家都在划水"
    )
...
def test_social_graph():
    ...
    sg.add_note("p1", "像洗衣妇")
    sg.record_claim(
        "p1", "fortune_teller", "self_claim", day_number=1, round_number=1, speaker_name="Alice"
    )
    sg.record_claim(
        "p1", "fortune_teller", "denial", day_number=2, round_number=2, speaker_name="Alice"
    )
...
def test_social_graph_freeze_thaw_and_claim_dedup():
    ...
    sg.record_claim(
        "p1", "fortune_teller", "self_claim", day_number=1, round_number=1, source_text="我跳占卜师"
    )
    sg.record_claim(
        "p1", "fortune_teller", "self_claim", day_number=1, round_number=1, source_text="我跳占卜师"
    )
...
def test_working_memory_private_info_has_priority_lane():
    ...
    wm.remember_objective_info(
        "evil_teammates", "你的邪恶队友是：Player 2", day_number=1, round_number=1
    )
    wm.remember_objective_info(
        "evil_bluffs", "说书人给邪恶阵营的 bluff 是：洗衣妇, 图书馆员", day_number=1, round_number=1
    )
    wm.remember_private_info("night_info", "你昨晚查出 Bob 不是恶魔", day_number=1, round_number=1)
...
def test_working_memory_clear_transient_preserves_all_memory_tiers():
    ...
    wm.remember_private_info(
        "undertaker_info",
        "送葬者信息: 今天被处决的玩家身份是：小恶魔。",
        day_number=2,
        round_number=2,
    )
    wm.remember_public_info(
        "role_claim", "Charlie 公开跳身份为 占卜师", day_number=2, round_number=2
    )
    ...
    assert wm.get_private_memory_summaries("undertaker_info") == [
        "送葬者信息: 今天被处决的玩家身份是：小恶魔。"
    ]
...
def test_working_memory_multi_day_archives_preserve_private_and_public_layers():
    ...
    wm.clear_transient()
    wm.remember_public_info(
        "public_fact", "Bob 明确否认自己是 洗衣妇", day_number=3, round_number=1
    )
    wm.clear_transient()
